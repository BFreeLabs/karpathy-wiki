...
`raw/memories/` holds atomized memory snippets intended to be ingested by a memory MCP server (e.g. a vector store, a `mem0`-style service, or any custom retrieval layer). Each file is **one atomic fact, concept, or recall item** — the kind of thing someone would ask an agent: *"How does technique X work?"* or *"Which source covered topic Y?"* The format below is MCP-server-agnostic: any retrieval system that ingests markdown with YAML frontmatter will work.
