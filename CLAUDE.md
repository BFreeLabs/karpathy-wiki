...
> **PLACEHOLDER — replace this paragraph during the customize-template prompt.**
>
> Describe in one paragraph what this wiki is about: the kinds of sources you'll add, the kinds of pages you want, and any adjacent topics that are in scope when they materially affect the main one. Examples: *"AI tools and workflows for developers,"* *"food science and home cooking experiments,"* *"home-lab infrastructure and self-hosting,"* *"medieval history primary sources."*
...
**Project-specific subdirectories**: if your wiki needs structured tracking beyond standard source ingest (e.g., recipe testing, hardware benchmarks, paper reading lists), declare them here in this tree and put their methodology under `.instructions/projects/<name>/`. The `customize-template` prompt scaffolds these for you during onboarding. **Nothing project-specific belongs in `.instructions/core/`.**
...
| core | Upgrade From Template | `.instructions/core/prompts/upgrade-from-template.md` | Pull methodology updates from the upstream template repo |
